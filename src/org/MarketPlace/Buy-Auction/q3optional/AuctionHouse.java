...
public class AuctionHouse {
  ...
  public boolean buyNow(BuyNowProduct product,User user, int quantity) {
	  if(product.attemptToPurchase(user, quantity) ) {
		  ...
		  user.buy(product.getProductId(), quantity);
		  return true;
	  }
  ...
  public String displayUnsoldProducts() {
	  ...
	  if(unsoldProducts.isEmpty()) {
		  ...
		  for(HashMap.Entry<Product, User> p:unsoldProducts.entrySet()){
			  if(p.getKey() instanceof BuyNowProduct) {
				  BuyNowProduct product= (BuyNowProduct) p.getKey();
    		  
				  if(product.howManyPurchases()==0 || (product.getQuantity()-product.howManyPurchases())==0 || (!product.isProductSold())) {
					  text+= p.getKey().getProductId()+ " - "+ p.getKey().getProductName() +"\n";
				  }
				  else {
					  text+= p.getKey().getProductId()+ " - "+ p.getKey().getProductName() + " with " + (product.getQuantity()-product.howManyPurchases())
    					  + " remaining \n";
				  }
			 }
  ...
  public Boolean placeBid(BiddableProduct product,User user, Double bidValue) {
	  ...
	  if(forSaleProducts.containsKey(product) && product.attemptedToPurchase(user, bidValue)) {
